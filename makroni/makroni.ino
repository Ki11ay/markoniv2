...
void updateFirebase(float temp1, float temp2) {
  ...
  if (Firebase.setString(firebaseData, "/lastUpdate", timestamp)) {
    Serial.println("Timestamp updated");
  } else {
    Serial.println("Failed to update timestamp: " + firebaseData.errorReason());
  }
}
...
void loop() {
  ...
  if (currentMillis - lastSensorRead >= SENSOR_READ_DELAY) {
    ...
    if (readSensors(temp1, temp2)) {
      ...
      if (currentMillis - lastFirebaseUpdate >= FIREBASE_UPDATE_DELAY) {
        Firebase.setString(firebaseData, "/lastUpdate", "0");
        lastFirebaseUpdate = currentMillis;
      }
      delay(2000);
    }
